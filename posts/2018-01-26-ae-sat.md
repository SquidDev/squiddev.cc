...
of unique items which are both visually distinct but easily classifiable. For variables, we'll use leather helmets. We
...
cannot use a variable and its negation in the same solution. Thus, like for the truthy items, we create a recipe
converting our initial variable item to several "falsey item"s (or "negation item"s). As there is only one variable
item in the system, only one recipe can be used and so only truthy or falsey items can be used in the solution.
...
I'd like to finish off by thanking [Amélia], who got me going down this rabbit hole in the first place, and has been
...
[Amélia]: https://amelia.how/ "Amélia's personal website and blog"
